...
async def example_basic_chat():
    ...
    messages = [Message(role="user", content="What is OpenRouter?")]
...
async def example_streaming_chat():
    ...
    async for chunk in provider.generate_messages(messages=messages, model="anthropic/claude-3-haiku", max_tokens=100):
...
async def example_different_models():
    ...
    for model in models:
        try:
            response = await provider.generate_message(messages=messages, model=model, max_tokens=10)
...
async def example_tool_calling():
    ...
    response = await provider.generate_message(
        messages=messages, model="openai/gpt-4", tools=[get_weather_tool], max_tokens=100
    )
...
async def main():
    ...
    try:
        ...
        print("\n(i) To run more examples, uncomment them in the main() function")
        ...
        print(f"Error: {e}")
        print("\n(i) Make sure OPENROUTER_API_KEY is set in your environment")
