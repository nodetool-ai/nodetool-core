...
async def _open_model_source(
    ...
    for _ in range(max_redirects + 1):
        ...
        resp = await session.get(current_url, headers=current_headers, allow_redirects=False)
        ...
        if resp.status in (301, 302, 303, 307, 308):
            ...
            if urlparse(new_url).hostname != parsed.hostname:
    ...
    raise ComfyError(f"Too many redirects while downloading model from {url}")
