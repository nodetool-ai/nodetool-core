...
# -----------------------------
def _pkg_run(cmd: list[str]) -> None:
    import subprocess
...
def _pkg_find_latest_wheel(dist_dir: str = "dist") -> Path:
    dist = Path(dist_dir)
...
def _pkg_read_pyproject_version(pyproject_path: Path) -> str:
    import tomllib

    with pyproject_path.open("rb") as f:
        data = tomllib.load(f)
    return str(data["project"]["version"])  # raises if missing (desired)


def _pkg_generate_sidecar(wheel_path: Path) -> Path:
    import zipfile
...
@package.command("build-wheel")
@click.option("--expected-version", help="Expected version to enforce")
@click.option("--skip-validate", is_flag=True, help="Skip twine validation")
@click.option("--skip-sidecar", is_flag=True, help="Skip PEP 658 sidecar generation")
def package_build_wheel(
    expected_version: str | None, skip_validate: bool, skip_sidecar: bool
):
    """Build wheel (Hatchling backend via python -m build), validate, and create sidecar."""
    # Optional version check
    if expected_version:
        actual_version = _pkg_read_pyproject_version(Path("pyproject.toml"))
        if actual_version != expected_version:
            raise SystemExit(
                f"Version mismatch: pyproject.toml has {actual_version}, expected {expected_version}"
            )

    # Build wheel using standard build frontend
    _pkg_run([sys.executable, "-m", "build", "--wheel"])  # outputs to dist/
    wheel = _pkg_find_latest_wheel("dist")
    click.echo(f"✅ Built wheel: {wheel.name}")

    if not skip_validate:
        _pkg_run(["twine", "check", str(wheel)])
        click.echo("✅ Twine validation passed")

    if not skip_sidecar:
        sidecar = _pkg_generate_sidecar(wheel)
        click.echo(f"✅ Sidecar created: {sidecar.name}")
...
@package.command("validate-wheel")
def package_validate_wheel():
    """Validate latest dist/*.whl with twine."""
    wheel = _pkg_find_latest_wheel("dist")
    _pkg_run(["twine", "check", str(wheel)])
    click.echo("✅ Twine validation passed")


@package.command("sidecar")
def package_sidecar():
    """Generate PEP 658 sidecar <wheel>.whl.metadata for latest wheel in dist/."""
    wheel = _pkg_find_latest_wheel("dist")
    sidecar = _pkg_generate_sidecar(wheel)
    click.echo(f"✅ Sidecar created: {sidecar.name}")


@package.command("release-notes")
@click.option("--package", "package_name", required=True)
@click.option("--version", required=True)
@click.option("--tag", required=True)
@click.option("--repository", required=True, help="owner/repo")
@click.option("--server-url", default="https://github.com")
def package_release_notes(
    package_name: str, version: str, tag: str, repository: str, server_url: str
):
    """Generate release_notes.md for the current project."""
    ...
    click.echo(f"📄 Generated release notes at {release_notes_path}")


@package.command("notify-registry")
@click.option("--package", "package_name", required=True)
@click.option("--version", required=True)
@click.option("--tag", required=True)
@click.option("--repository", required=True, help="owner/repo")
@click.option("--server-url", default="https://github.com")
@click.option("--registry-repo", default="nodetool-ai/nodetool-registry")
def package_notify_registry(
    package_name: str,
    version: str,
    tag: str,
    repository: str,
    ...
    """Send repository_dispatch to nodetool-registry about a released package."""
    import os
    import json
    ...
    try:
        with _req.urlopen(req) as resp:  # noqa: S310
            click.echo(f"📨 Registry notified (status {resp.status})")
    except _err.HTTPError as e:
        click.echo(f"⚠️ Failed to notify registry (non-fatal): {e}")
...
def package_summary(
    ...
    """Append a success summary to the GitHub step summary when available."""
    import os

    summary_path = os.getenv("GITHUB_STEP_SUMMARY")
    if not summary_path:
        click.echo("⚠️ GITHUB_STEP_SUMMARY not set; skipping summary")
        return
    with open(summary_path, "a", encoding="utf-8") as f:
        ...
        f.write("```\n")
    click.echo(f"📝 Wrote summary to {summary_path}")
