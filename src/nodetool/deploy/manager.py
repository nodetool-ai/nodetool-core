        ...
        for deployment_name in deployments_to_validate:
            try:
                ...
                if isinstance(deployment, SelfHostedDeployment):
                    # Validate SSH config
                    if (
                        hasattr(deployment, "ssh")
                        and deployment.ssh  # type: ignore[attr-defined]
                        and not deployment.ssh.key_path  # type: ignore[attr-defined]
                        and not deployment.ssh.password  # type: ignore[attr-defined]
                    ):
