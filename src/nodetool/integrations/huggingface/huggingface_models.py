...
def detect_repo_packaging(
    ...
    lower_sharding_candidates = lower_weight_files + [name.lower() for name, _ in file_entries if _is_index_file(name)]
...
async def get_text_to_video_models_from_hf_cache() -> list[VideoModel]:
    ...
    async for repo_id, _repo_dir, snapshot_dir, file_list in iter_cached_model_files():
        if not file_list:
            continue
        if _read_model_index_class_name(snapshot_dir) not in _VIDEO_PIPELINE_CLASS_NAMES:
            continue
        result.setdefault(
            repo_id,
            VideoModel(
...
async def get_text_to_audio_models_from_hf_cache() -> list[AudioModel]:
    ...
    async for repo_id, _repo_dir, snapshot_dir, file_list in iter_cached_model_files():
        if not file_list:
            continue
        if _read_model_index_class_name(snapshot_dir) not in _AUDIO_PIPELINE_CLASS_NAMES:
            continue
        result.setdefault(
            repo_id,
            AudioModel(
