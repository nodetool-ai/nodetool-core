...
class MultiprocessRunner:
    """
    A class that manages running workflows in a separate process.

    This class provides a way to execute workflows in a separate Python process,
    allowing for isolation and parallel execution. It handles:

    - Starting a workflow in a separate process
    - Communication between the main process and the workflow process via a queue
    - Streaming processing messages from the workflow back to the caller
    - Cancellation and cleanup of running workflows

    The separate process model ensures that workflow execution doesn't block the
    main application thread and provides isolation for workflow execution.

    Attributes:
        req (RunJobRequest): The request containing workflow execution parameters
        queue (multiprocessing.Queue): Queue for inter-process communication
        process (multiprocessing.Process): The separate process running the workflow
    """
