    ...
    async def audio_from_numpy(
        ...
        def _segment_from_numpy() -> Any:
            ...
            if data.dtype == np.int16:
                ...
                # ⚡ Bolt Optimization: Use .clip() instance method and np.float32 scalar to prevent float64 upcast
                data_bytes = (data.clip(-1.0, 1.0) * np.float32(32767.0)).astype(np.int16).tobytes()
            else:
