...
class SuspendableNode(BaseNode):
    ...
    def _set_resuming_state(self, saved_state: dict[str, Any], event_seq: int) -> None:
        ...
        log.debug(
            f"Node {self._id} set to resuming mode (event_seq={event_seq}, state_keys={list(saved_state.keys())})"
        )
...
class WorkflowSuspendedException(Exception):
    ...
    def __init__(self, node_id: str, reason: str, state: dict[str, Any], metadata: dict[str, Any]):
        self.node_id = node_id
        self.reason = reason
        self.state = state
        self.metadata = metadata

        super().__init__(f"Workflow suspended at node {node_id}: {reason}")
