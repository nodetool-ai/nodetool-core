"""
RunProjection model for materialized workflow state.

This model represents a projection (materialized view) of workflow execution state
derived from the event log. Projections can be rebuilt from scratch by replaying
events, enabling recovery and consistency guarantees.
"""
...
class RunProjection(DBModel):
    ...
    This projection is rebuilt from the event log and provides efficient
    access to current run state without scanning all events. It can be
