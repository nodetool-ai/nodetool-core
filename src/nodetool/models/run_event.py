...
This model implements the event sourcing pattern for workflow execution.
Each event represents a durable state transition in the workflow lifecycle.
"""
...
class RunEvent(DBModel):
    ...
    Events form an append-only log that serves as the source of truth for
    workflow state. All other state (projections, caches) is derived from events.
    
    Key invariants:
    - Events are immutable once written
    - Sequence numbers are monotonic per run
    - Event append operations are idempotent
    """
