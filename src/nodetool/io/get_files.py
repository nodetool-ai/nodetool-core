...
def get_files(path: str, extensions: list[str] | None = None):
    ...
    if os.path.isdir(path):
        for file in os.listdir(path):
            # ⚡ Bolt Optimization: Use .extend() instead of += to avoid O(n^2) list reallocation
...
def get_content(
    ...
    """
    extensions = extensions or [".py", ".js", ".ts", ".jsx", ".tsx", ".md"]
    # ⚡ Bolt Optimization: Use list.append() and "".join() to avoid O(n^2) string concatenation
    content_parts = []
    for path in paths:
