...
class CreateWorkflowCommand(Command):
    ...
    async def execute(self, cli: ChatCLI, args: List[str]) -> bool:
        ...
        try:
            ...
            try:
                async for update in planner.create_graph(cli.context):
                    if isinstance(update, PlanningUpdate):
                        status_color = "green" if update.status == "Success" else "yellow" if update.status == "Starting" else "red"
            ...
            workflow_name, workflow_description = await self._get_workflow_name_and_description(cli, objective)
