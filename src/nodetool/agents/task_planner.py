...
class TaskPlanner:
    ...
    **Validation is paramount:** The planner rigorously checks the generated plan for:
    ...
    async def _run_plan_creation_phase(
        ...
        Chunk | ToolCall | PlanningUpdate | tuple[Optional[Task], Optional[Exception], Optional[PlanningUpdate]], None
    ]:
                ...
                if attempt < max_retries - 1:
                    ...
                    if last_msg and last_msg.role == "assistant" and last_msg.tool_calls:
                        for tc in last_msg.tool_calls:
                            history.append(
                                Message(
                                    role="tool", tool_call_id=tc.id, content=f"Error during plan generation: {str(e)}"
                                )
                            )
                    else:
                        history.append(
