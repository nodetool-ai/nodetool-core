...
**Note: All file paths (`input_files`, `output_file`, `artifacts`) can refer to either files or directories.**
...
2. **Data Flow (Phase 1):** Define precise data dependencies (`input_files`, `output_file`, `artifacts`) and result passing (prefer file pointers for large data).
...
- **Relative Paths:** All file paths (`output_file`, `artifacts`, `input_files`) MUST be relative (e.g., `data/output.csv` or `data/processed/`). No absolute paths or `workspace/` prefix. These paths can point to files or directories.
- **Unique File Paths:** Each `output_file` and `artifact` path MUST be unique across the entire plan.
...
- Consider relative file paths (files or directories), dependencies, and how results will be passed (content vs. file pointer - prefer pointers for larger data).
- **Estimate Complexity:** Briefly assess the likely complexity of each subtask (simple, moderate, complex) to inform later iteration limits.
- Plan for separate subtasks for each input file/item if processing independently.
- Design for minimal execution context for each subtask.
- **Define concise `content` instructions for each agent, focusing only on the *what* (objective), not the *how* (specific steps).**
...
+Before providing the CSN output, explain your analysis and reasoning. Describe how you broke down the objective, identified conceptual subtasks, considered parallel opportunities, and planned the initial data flow. Include any assumptions made or alternative approaches considered.
+Refer to conceptual tasks using the '$name' convention.
+As part of your thought process, mentally construct or describe the data flow using the DOT concept (e.g., "users.json" -> "$merge_step" -> "$filter_step").

Consider the data flow using the DOT visualization concept.
...
**OUTPUT TEMPLATE:**
1. **Task List:**
List each subtask with a $short_name and description.
2. **Reasoning:**
Reason about the objective and reference tasks by their $short_name.
3. **DOT/Graphviz Syntax:**
Mentally construct or describe the data flow using the DOT concept (e.g., "users.json" -> "$merge_step" -> "$filter_step").
...
- **Finalize Data Flow:** Determine the definitive `input_files` and `output_file` based on dependencies between subtasks. Note that these paths can be files or directories.
...
3. **Task List:**
List each subtask with a $short_name and description, input and output files.
"""
...
4.  Inside the `create_task` tool call, for each parsed subtask:
    ...
    *   Set `artifacts` (likely an empty list unless inferred).
...
2. Define precise relative file paths (`input_files`, `output_file`, `artifacts`) extracted/determined from CSN/context. No absolute paths or `/workspace/` prefix. Unique outputs.
...
4. **Unique File Paths:** Ensure *all* generated file paths (`output_file`, `artifacts`) are unique.
...
10. **Minimize Context:** Ensure subtasks remain focused. Prefer passing results via file pointers (`{"path": "..."}`) to keep context small.
11. **Final Output Conformance:** Ensure the plan's final output conforms to the overall task requirements (Type: `{{ overall_output_type }}`, Schema: `{{ overall_output_schema }}`) and is saved to `output_files/`.
12. **Reasoning:** If the task requires reasoning, use the `reasoning` tool (This applies to the agent executing the task, not the planner).
...
CREATE YOUR PLAN NOW using the `create_task` tool. Parse the CSN lines from Phase 1, determine output types and artifacts, ensure *minimal* agent instructions (`content`), appropriate `max_iterations`, prefer file pointers for data transfer, and verify the final output meets requirements by constructing the correct JSON for `create_task`.
"""
...
3. Well-defined relative paths (`input_files`, `output_file`, `artifacts`). No absolute paths or `/workspace/` prefix. Unique outputs. These paths can refer to files or directories.
    ...
    **Validation is paramount:** The planner rigorously checks the generated plan for:
    ...
    async def _run_analysis_phase(
        ...
        if not self.enable_analysis_phase:
            self._update_planning_table(
                table, "0. Analysis", "Skipped", "Phase disabled."
            )
            return history, None
        ...
        self._update_planning_table(table, "0. Analysis", phase_status, phase_content)
    ...
    async def _run_data_flow_phase(
        ...
        if not self.enable_data_contracts_phase:
            self._update_planning_table(
                table, "1. Data Contracts", "Skipped", "Phase disabled."
            )
            return history, None
        ...
        self._update_planning_table(
            table, "1. Data Contracts", phase_status, phase_content
        )
    ...
    async def create_task(
        ...
        try:
            ...
            history, planning_update = await self._run_analysis_phase(history, table)
            if planning_update:
                yield planning_update

            # Phase 1: Data Flow Analysis
            current_phase = "Data Flow"
            history, planning_update = await self._run_data_flow_phase(history, table)
            if planning_update:
                yield planning_update
