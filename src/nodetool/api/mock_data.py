...
async def generate_mock_threads(
    user_id: str = "1", count: int = 5
) -> list[Thread]:
...
async def generate_mock_messages(
    thread_id: str, user_id: str = "1", count: int = 10
) -> list[Message]:
    ...
    for i in range(count):
        ...
        if is_user:
            content: Any = [
                MessageTextContent(text=random.choice(SAMPLE_USER_MESSAGES))
            ]
            role = "user"
            provider = None
            model = None
...
async def generate_mock_workflows(
    user_id: str = "1", count: int = 5
) -> list[Workflow]:
...
async def generate_mock_assets(
    user_id: str = "1", count: int = 10
) -> list[Asset]:
...
async def populate_mock_data(user_id: str = "1") -> dict[str, Any]:
    ...
    for thread in threads[:3]:  # Only add messages to first 3 threads
        messages = await generate_mock_messages(
            thread_id=thread.id, user_id=user_id, count=10
        )
