...
class AsyncTaskGroup:
    ...
    Useful for executing multiple async operations concurrently with error handling.
    ...
    Example:
        ...
        # Spawn tasks
        group.spawn("task1", fetch_data("https://api.example.com/data1"))
        group.spawn("task2", fetch_data("https://api.example.com/data2"))
        group.spawn("task3", fetch_data("https://api.example.com/data3"))
    ...
    async def run(
        ...
        *,
        ...
        all_results = [r for r in self._results if not r.success]

        if raise_on_error and all_results:
            raise TaskExecutionError(
                f"{len(all_results)} task(s) failed during execution",
            ...
            if failed:
                for r in self._results:
                    if r.exception is not None:
                        raise r.exception  # type: ignore[misc]
        if exc_type is not None:
            raise exc_val  # type: ignore[misc]


__all__ = ["AsyncTaskGroup", "TaskExecutionError", "TaskResult", "TaskStats"]
