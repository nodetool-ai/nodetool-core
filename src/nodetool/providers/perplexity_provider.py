"""
Perplexity AI provider implementation for chat completions.

This module implements the ChatProvider interface for Perplexity AI,
which provides access to Perplexity's language models through an OpenAI-compatible API.
...
@register_provider(Provider.Perplexity)
class PerplexityProvider(OpenAIProvider):
    """Perplexity AI implementation of the ChatProvider interface.

    Perplexity AI provides access to specialized language models including sonar models
    designed for search-augmented generation through an OpenAI-compatible API.

    Key differences from OpenAI:
    1. Base URL: https://api.perplexity.ai
    2. Uses PERPLEXITY_API_KEY for authentication
    3. Different set of available models (sonar models, llama models, etc.)
    4. Some models may not support all OpenAI features

    For details, see: https://docs.perplexity.ai/api-reference/chat-completions
    """

    provider: Provider = Provider.Perplexity
    ...
    def __init__(self, secrets: dict[str, str]):
        """Initialize the Perplexity provider with client credentials.

        Reads ``PERPLEXITY_API_KEY`` from secrets.
        """
        assert "PERPLEXITY_API_KEY" in secrets, "PERPLEXITY_API_KEY is required"
        self.api_key = secrets["PERPLEXITY_API_KEY"]
        self.client = None
        self.cost = 0.0
        log.debug("PerplexityProvider initialized. API key present: True")

    def get_container_env(self, context: ProcessingContext) -> dict[str, str]:
        """Return environment variables required for containerized execution.

        Returns:
            A mapping containing ``PERPLEXITY_API_KEY`` if available; otherwise empty.
        """
        return {"PERPLEXITY_API_KEY": self.api_key} if self.api_key else {}

    def get_client(self) -> openai.AsyncClient:
        """Create and return a Perplexity async client.

        Uses OpenAI SDK with Perplexity's base URL and API key.

        Returns:
            An initialized ``openai.AsyncClient`` configured for Perplexity.
        """
        log.debug("Creating Perplexity async client")
        ...
        # Configure client for Perplexity
        client = openai.AsyncClient(
            api_key=self.api_key,
            base_url="https://api.perplexity.ai",
            http_client=http_client,
        )
        log.debug("Perplexity async client created successfully")
        return client

    def has_tool_support(self, model: str) -> bool:
        """Return True if the given model supports tools/function calling.

        Perplexity models have varying support for function calling.
        Some models support it while others do not.
        ...
        # Perplexity sonar models and some llama models support function calling
        # For simplicity, we'll return True for most models
        # Users should consult Perplexity docs for specific model capabilities
        log.debug(f"Model {model} assumed to support tool calling")
        return True
    ...
    async def generate_message(  # type: ignore[override]
        ...
        **kwargs,
        ...
        """Generate a non-streaming completion from Perplexity.

        This method extends OpenAI's generate_message for Perplexity.
        ...
        log.debug("Making non-streaming API call to Perplexity")
    ...
    async def generate_messages(  # type: ignore[override]
        ...
        **kwargs,
        ...
        """Stream assistant deltas and tool calls from Perplexity.

        This method extends OpenAI's generate_messages for Perplexity.
        ...
        log.debug(f"Converting {len(messages)} messages to OpenAI format")
        openai_messages = [await self.convert_message(m) for m in messages]
        log.debug("Making streaming API call to Perplexity")
