...
class RodinProvider(BaseProvider):
    ...
    async def _poll_task_status(
        ...
        for attempt in range(max_attempts):
            ...
            all_done = True
            for job in jobs:
                status = job.get("status", "").lower() if isinstance(job, dict) else str(job).lower()
    ...
    async def text_to_3d(
        ...
        Cost: 0.5 credits
        """
        log.info("Generating 3D with Hyper3D Rodin (text-to-3D)")
    ...
    async def image_to_3d(
        ...
        Cost: 0.5 credits
        """
        log.info("Generating 3D with Hyper3D Rodin (image-to-3D)")
