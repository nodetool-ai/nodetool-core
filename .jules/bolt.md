...
## 2025-08-01 - Prevent implicit float64 upcasting during array scaling and clipping
**Learning:** Using module-level `np.clip()` and implicitly multiplying NumPy arrays by standard Python constants (like `32767` or `32768.0`) causes unnecessary upcasting to `float64`, particularly when the data is originally `float16` or `float32`. This doubles memory footprint and drastically reduces performance. The module-level `np.clip()` also introduces `__array_function__` dispatcher overhead.
**Action:** When scaling and clipping NumPy arrays, explicitly normalize the inputs to `float32` using `np.asarray(data, dtype=np.float32)`, use the faster instance method `.clip()`, and multiply by an explicitly typed scalar (e.g. `np.float32(32767.0)`).
