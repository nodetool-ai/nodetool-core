...
## 2024-05-14 - Optimize repetitive string concatenation using lists and join()
**Learning:** In CPython, iterative string concatenation using `+=` is optimized for simple loops. However, in scenarios involving many iterations or complex/nested loops (e.g., aggregating file contents), repeatedly using `+=` forces Python to allocate new string objects, resulting in an O(N^2) byte-copying penalty. Using a list to collect string fragments and calling `"".join(list)` is significantly faster (O(N) time complexity) and avoids this massive performance overhead.
**Action:** Always favor appending to a list and calling `"".join(list)` over `+=` for string concatenation when the number of iterations is large or unknown, to prevent performance degradation.
