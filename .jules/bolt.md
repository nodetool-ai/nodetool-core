...
## 2025-08-10 - Prevent float16 overflow during array scaling
**Learning:** When using explicit scalar typing (e.g. `dt(32767)`) to prevent NumPy `float64` upcasting during array scaling to `int16`, casting large constants directly to `float16` is dangerous. `float16` lacks the precision to represent `32767` exactly and rounds up to `32768`. Multiplying by `32768` and casting to `int16` causes an integer overflow and wraps around to `-32768`.
**Action:** When scaling NumPy arrays to `int16`, explicitly check for `float16` inputs and promote the scalar dtype to `float32` (e.g. `dt = np.float32 if arr.dtype == np.float16 else arr.dtype.type`) before the multiplication to prevent overflow while still avoiding the `float64` upcast overhead.
