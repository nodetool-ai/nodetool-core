...
## 2024-05-18 - Audio Scaling Float16 Overflow Optimization
**Learning:** While explicitly casting arrays to their native type before clipping and multiplying by constants (e.g., `dt = arr.dtype.type; (arr.clip(dt(-1.0), dt(1.0)) * dt(32767.0))`) successfully prevents implicit `float64` upcasting, it introduces a critical overflow bug for `float16` arrays. Because `32767.0` cannot be perfectly represented in `float16`, it rounds up to `32768.0`. When `float16` arrays with maximum amplitudes (`1.0`) are multiplied, the result hits `32768`, which overflows `int16`'s maximum bounds and wraps around to `-32768`, causing extreme audio distortion.
**Action:** When scaling audio arrays across float types to `int16` bounds, explicitly cast and perform the math using `np.float32` (e.g., `dt = np.float32; (arr.astype(dt, copy=False).clip(dt(-1.0), dt(1.0)) * dt(32767.0))`). This safely prevents `float64` upcasting while providing enough precision for the `32767.0` multiplier across `float16`, `float32`, and `float64` types.
