...
## 2026-08-05 - Avoid float64 upcasting and emulated float16 in audio array clipping
**Learning:** Multiplying a NumPy float32 array by a Python float scalar (like `32767.0`) triggers implicit upcasting to `float64`, doubling memory footprint. Using `.astype(np.float32, copy=False)` to prevent this breaks strictly in NumPy 2.0+ if the array is `float64` or `float16`, as `copy=False` will raise a ValueError.
**Action:** Always use `np.asarray(data, dtype=np.float32)` prior to `clip` and arithmetic scaling. This acts as a zero-copy view if the data is already `float32`, safely performs the necessary copy/cast for `float64` to halve memory overhead, and upgrades emulated `float16` to native `float32` for significantly faster hardware arithmetic.
