...
## 2026-08-18 - Optimize NumPy float32 to int16 conversion using array.clip() and explicit scalar typing
**Learning:** Using the module-level `np.clip(arr, ...)` is slower than using the instance method `arr.clip(...)`. Furthermore, explicitly casting numpy arrays to `np.float32` via `np.asarray` and multiplying by a `np.float32` typed scalar avoids NumPy implicitly upcasting data to `float64` during operations, halving memory usage and substantially improving performance, especially on `float16` inputs.
**Action:** When working with float NumPy arrays to convert them to audio `int16`, explicitly normalize inputs to `float32` and use explicit `np.float32(...)` scalars. Use `arr.clip()` rather than `np.clip(arr, ...)`.
