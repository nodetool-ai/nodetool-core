...
## 2026-08-10 - Prevent implicit float64 upcasting during audio clipping
**Learning:** In NumPy 2.0+, operations between a `float32` NumPy array and standard Python scalars (like `32767` or `1.0`) inside functions like `np.clip` or during arithmetic trigger an implicit upcast to `float64`. This doubles memory usage and halves execution speed, particularly harmful when processing large audio arrays to raw `int16` bytes.
**Action:** When converting floating-point audio data to `int16`, avoid `np.clip(data, -1.0, 1.0) * 32767`. Instead, explicitly ensure `float32` type and use scalar types: `np.asarray(data, dtype=np.float32).clip(-1.0, 1.0) * np.float32(32767.0)`. This bypasses `__array_function__` overhead and keeps operations in fast 32-bit float space before final `astype(np.int16)`.
