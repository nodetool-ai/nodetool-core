...
## 2025-08-01 - NumPy arr.clip instance method and explicit float32 scaling
**Learning:** Using the instance method `arr.clip(min, max)` is significantly faster than using the module-level function `np.clip(arr, min, max)` because it avoids NumPy's internal dispatcher and argument parsing overhead. Furthermore, when scaling `float32` arrays by constants (like `32767`), using a standard Python integer or float (e.g. `32767` or `32767.0`) triggers an implicit upcast to `float64`, effectively doubling memory usage and increasing computation time. Using an explicit `np.float32(32767.0)` maintains `float32` precision.
**Action:** Always prefer the instance method `.clip(...)` for NumPy array clipping. When multiplying a `float32` array by a constant, cast the constant to `np.float32` to prevent implicit `float64` upcasting.
