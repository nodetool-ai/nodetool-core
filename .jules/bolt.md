...
## 2025-07-21 - PyTorch Tensor to NumPy Image Conversion Optimization
**Learning:** When converting a PyTorch tensor to a `uint8` NumPy image array (e.g. `tensor_to_image_array`), performing the multiplication and clipping in NumPy after conversion causes implicit `float64` upcasting when multiplying by `255.0` (unless explicitly typed). More importantly, it creates unnecessary intermediate NumPy arrays and uses less efficient operations than PyTorch.
**Action:** When scaling and clipping PyTorch tensors for image conversion (e.g., to uint8 NumPy arrays), perform the math operations in-place on the PyTorch tensor using `.float().mul(255.0).clamp_(0, 255).byte().numpy()` before converting to NumPy. To preserve correctness and avoid regressions compared to NumPy's implicit float upcasting, ensure non-floating point tensors (like integer or boolean) are explicitly cast to float first using `.float()` before applying the multiplication.
