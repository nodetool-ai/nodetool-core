...
## 2025-08-30 - Optimize dynamic membership checks in loops
**Learning:** When performing dynamic membership checks inside a loop (e.g., `if item not in [e.value for e in edges]:`), evaluating a list comprehension repeatedly requires $O(N)$ array construction and $O(M)$ lookup per iteration, resulting in $O(N \times M)$ complexity. By extracting the target sequence into a pre-computed set outside the loop using a set comprehension (e.g., `target_set = {e.value for e in edges}`), we reduce redundant array constructions and improve lookup complexity to $O(N + M)$ because set lookups are $O(1)$.
**Action:** When membership checks are required inside a loop against a dynamically derived sequence, hoist the computation out of the loop and store the results in a set comprehension to optimize performance.
