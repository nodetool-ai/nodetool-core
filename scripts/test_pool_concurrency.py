    ...
    try:
        ...
        async with pool.acquire_context() as conn:
            ...
            if task_id % 10 == 0:
                ...
                await conn.execute(
                    "INSERT OR REPLACE INTO test_concurrency (id, task_id) VALUES (?, ?)", (task_id % 100, task_id)
                )
                await conn.commit()
...
async def main():
    ...
    try:
        ...
        other_errors = sum(
            1 for r in results if "FAIL" in r and "locked" not in r.lower() and "closed" not in r.lower()
        )
