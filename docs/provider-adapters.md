...
NODETOOL_PROVIDER_ADAPTER_CAPABILITIES_EXAMPLE=text_to_video,image_to_video
...
receives `provider`, `operation`, and `params`. `image_to_video` additionally
receives an absolute `image_path` owned by the worker and valid only for that
request. A successful video operation returns an existing output `path`.

The worker reads that file and transfers the video through the authenticated
