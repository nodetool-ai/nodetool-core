...
- **Workflow Building Guide**: Use the `build_workflow_guide` prompt for complete workflow architecture patterns, node types, execution methods, and best practices
...
2. **`run_workflow_tool`** - Execute saved workflows synchronously (best for quick operations)
   - Use when: Running tested workflows that complete in seconds
   - Blocks until completion
   - Returns results immediately

3. **`start_background_job`** - Execute long-running workflows asynchronously
...
- Workflows MUST have terminal nodes to capture results - use Output, Preview, or Save nodes (processing alone doesn't return values)
...
2. **Use Type Filters**: Specify `input_type` and `output_type` when known
3. **Batch Related Searches**: Search for multiple capabilities together
4. **Target Namespaces**:
...
**NodeTool implements iteration through streaming.** Streaming nodes emit multiple values sequentially, and downstream nodes automatically execute once per emitted value. This is how loops and batch processing work.
...
**Critical Concept:** NodeTool has no traditional loop constructs. Instead, **iteration is accomplished through streaming.**

### How Streaming Creates Iteration

1. **Stream Generators** emit values sequentially (one after another)
2. **Downstream nodes** automatically execute once for each emitted value
3. **Outputs accumulate** - each execution adds to the result list
...
- **Terminal nodes required**: You MUST use terminal nodes to capture workflow results:
  ...
  - **Save to Assets**: `nodetool.image.SaveImage`, `nodetool.audio.SaveAudio`, `nodetool.text.SaveText`, `nodetool.video.SaveVideo`, `nodetool.data.SaveDataframe`
  - **Save to File**: `nodetool.image.SaveImageFile`, `nodetool.audio.SaveAudioFile`, `nodetool.text.SaveTextFile`, `nodetool.video.SaveVideoFile`, `nodetool.data.SaveCSVDataframeFile`
...
2. Identify outputs (types, destinations)
3. Map transformation steps
4. Consider error cases and edge conditions
5. **Write requirements**: `/tmp/nodetool_${SESSION_ID}_requirements.md`

### Phase 2: Design Data Flow
1. Create Input nodes first
2. Search for processing nodes (use filters!)
3. Plan connections (trace data flow inputs → processing → outputs)
4. Identify dynamic properties needs (templates, dictionaries)
5. Plan for streaming if generating multiple items
6. **Write design document**: `/tmp/nodetool_${SESSION_ID}_design.md`
7. **Write search log**: `/tmp/nodetool_${SESSION_ID}_searches.json`

### Phase 3: Build & Test
1. **For Testing**: Use `run_graph` - no need to save first
2. Create node specifications with exact types from `search_nodes`
3. Define all edges between nodes
4. Validate structure (DAG, unique IDs, type compatibility)
5. **Write workflow JSON**: `/tmp/nodetool_${SESSION_ID}_graph.json`
6. **For Production**: Use `save_workflow` then `run_workflow_tool` or `start_background_job`
...
**File**: `/tmp/nodetool_${SESSION_ID}_requirements.md`
**Write after**: Phase 1
...
### 2. Design Document
**File**: `/tmp/nodetool_${SESSION_ID}_design.md`
**Write after**: Phase 2
...
### 3. Node Search Log
**File**: `/tmp/nodetool_${SESSION_ID}_searches.json`
**Write during**: Phase 2 (update after each search)
...
### 4. Workflow Graph
**File**: `/tmp/nodetool_${SESSION_ID}_graph.json`
**Write after**: Phase 3
...
### 5. Execution Results
**File**: `/tmp/nodetool_${SESSION_ID}_results.json`
**Write after**: Phase 4
...
### 6. Summary Report
**File**: `/tmp/nodetool_${SESSION_ID}_summary.md`
**Write after**: Phase 4 (final step)
...
2. **Write Early**: Create files as soon as you complete each phase
3. **Update Incrementally**: Update search log after each search
4. **Include Timestamps**: Add ISO timestamps to all files
5. **Preserve Decisions**: Document why you chose specific nodes/patterns
6. **Enable Inspection**: Make files human-readable for debugging
7. **Track Changes**: If requirements change mid-design, note the evolution
8. **Print File Paths**: Always output the full file paths after writing
...
2. **Explain Choices**: Justify node selections and patterns used
3. **Show Examples**: Reference similar patterns from `workflow_examples` prompt
4. **Test First**: Use `run_graph` for rapid prototyping
5. **Save When Ready**: Use `save_workflow` only for production-ready workflows
6. **Provide Monitoring Guidance**: Explain how to track execution
...
2. Type mismatches without converters
3. Using wrong node_type string
4. Orphaned nodes not connected to data flow
5. **Forgetting terminal nodes** - workflows won't return results without Output/Preview/Save nodes
6. Template variables without edges
7. Creating circular dependencies
8. Too many search iterations (plan searches better!)

Remember: Your goal is to create workflows that are elegant, maintainable, and efficient. Use `run_graph` for rapid testing, strategic node searching to minimize iterations, and the comprehensive MCP guides to ensure best practices.
