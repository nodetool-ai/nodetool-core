- **2026-01-15**: Added `AsyncRateLimiter` class implementing token bucket algorithm for time-based rate limiting in `src/nodetool/concurrency/rate_limiter.py`, with `RateLimitConfig` dataclass and `rate_limited` helper function.
