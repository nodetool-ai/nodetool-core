...
#     with patch('nodetool.chat.chat_websocket_runner.Environment.use_remote_auth', return_value=False):
#         # Connect without auth (in test mode)
#         await runner.connect(websocket) # type: ignore
#
#         # Mock the provider to raise httpx.ConnectError
#         with patch('nodetool.chat.chat_websocket_runner.get_provider') as mock_get_provider:
