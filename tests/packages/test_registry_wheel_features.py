...
class TestInstallPackageWithFallback:
    ...
    def test_install_package_wheel_fallback_to_git(self, mock_registry):
        ...
        with patch.object(mock_registry, 'check_package_index_available', return_value=True), \
             ...
             patch.object(mock_registry, 'pip_install', side_effect=[subprocess.CalledProcessError(1, 'pip'), None]) as mock_pip:
            ...
            mock_pip.assert_any_call(f"git+https://github.com/{repo_id}", use_index=False)
    ...
    def test_install_package_index_unavailable_use_git(self, mock_registry):
        ...
        with patch.object(mock_registry, 'check_package_index_available', return_value=False), \
            ...
            mock_pip.assert_called_once_with(f"git+https://github.com/{repo_id}", use_index=False)

    def test_install_package_force_git(self, mock_registry):
        """Test forcing git-based installation."""
        repo_id = "nodetool-ai/nodetool-base"
        
        with patch.object(mock_registry, 'pip_install') as mock_pip, \
             patch.object(mock_registry, 'clear_packages_cache'):
...
class TestUpdatePackageWithFallback:
    ...
    def test_update_package_wheel_success(self, mock_registry):
        ...
        with patch.object(mock_registry, 'check_package_index_available', return_value=True), \
            ...
            assert result is True
            mock_pip.assert_called_once_with("nodetool-base", upgrade=True, use_index=True)
            mock_clear.assert_called_once()
    ...
    def test_update_package_wheel_fallback_to_git(self, mock_registry):
        ...
        with patch.object(mock_registry, 'check_package_index_available', return_value=True), \
            ...
            mock_pip.assert_any_call(f"git+https://github.com/{repo_id}", upgrade=True, use_index=False)
    ...
    def test_update_package_force_git(self, mock_registry):
        ...
        with patch.object(mock_registry, 'pip_install') as mock_pip, \
            ...
            assert result is True
            mock_pip.assert_called_once_with(f"git+https://github.com/{repo_id}", upgrade=True, use_index=False)
    ...
    def test_update_package_subprocess_error(self, mock_registry):
        ...
        with patch.object(mock_registry, 'check_package_index_available', return_value=False), \
             patch.object(mock_registry, 'pip_install', side_effect=subprocess.CalledProcessError(1, 'pip')):
...
class TestErrorHandling:
    ...
    def test_pip_install_preserves_existing_behavior(self, mock_registry):
        ...
        with patch('subprocess.check_call') as mock_subprocess:
            ...
            expected_cmd = [*mock_registry.pkg_mgr, "install", "file:///path/to/package"]
            mock_subprocess.assert_called_once_with(expected_cmd)
...
class TestIntegrationWithExistingCode:
    ...
    def test_wheel_features_dont_break_existing_methods(self, mock_registry):
        ...
        with patch.object(mock_registry, 'pip_uninstall') as mock_uninstall:
            result = mock_registry.uninstall_package("owner/project")
...
class TestLogging:
    ...
    def test_install_fallback_logs_warning(self, mock_registry, caplog):
        ...
        with patch.object(mock_registry, 'check_package_index_available', return_value=True), \
             patch.object(mock_registry, 'clear_packages_cache'), \
             patch.object(mock_registry, 'pip_install', side_effect=[subprocess.CalledProcessError(1, 'pip'), None]):
