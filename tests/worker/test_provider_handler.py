...
from nodetool.worker.provider_handler import (
    _reference_media_suffix,
    _text_to_audio_kwargs,
)
from nodetool.worker.server import WorkerServer, start_server


def test_reference_video_accepts_real_iso6_mp4_and_rejects_image_brands():
    iso6 = (Path(__file__).parent / "fixtures" / "iso6.mp4").read_bytes()
    assert _reference_media_suffix(iso6, "video") == ".mp4"
    for brand in (b"avif", b"heic", b"heix", b"mif1"):
        data = b"\x00\x00\x00\x18ftyp" + brand + b"\x00\x00\x00\x00"
        assert _reference_media_suffix(data, "video") is None


@pytest.mark.parametrize(("brand", "suffix"), [(b"iso5", ".mp4"), (b"qt  ", ".mov")])
def test_reference_video_accepts_common_iso_bmff_video_brands(brand: bytes, suffix: str):
    data = b"\x00\x00\x00\x18ftyp" + brand + b"\x00\x00\x00\x00"
    assert _reference_media_suffix(data, "video") == suffix
...
""".strip()
    ...
    host, port = server
    async with websockets.connect(f"ws://{host}:{port}") as ws:
        await ws.send(msgpack.packb({"type": "provider.list", "request_id": "list", "data": {}}))
...
""".strip()
    ...
    monkeypatch.setenv("NODETOOL_PROVIDER_ADAPTER_CAPABILITIES_EXTERNAL", capability)
    ...
    async with websockets.connect(f"ws://{host}:{port}") as ws:
        ...
        while not frames or frames[-1]["type"] != "result":
            frames.append(msgpack.unpackb(await asyncio.wait_for(ws.recv(), 5), raw=False))
...
""".strip()
    ...
    monkeypatch.setenv("NODETOOL_PROVIDER_ADAPTER_CAPABILITIES_EXTERNAL", "text_to_video")
    ...
    async with websockets.connect(f"ws://{host}:{port}") as ws:
        ...
        await ws.send(msgpack.packb({"type": "cancel", "request_id": "cancel-me", "data": {}}))
...
assert pathlib.Path(videos[0]).read_bytes()[:12] == bytes.fromhex("000000206674797069736f36")
            ...
            msgpack.packb(
                {
                    ...
                    "data": {
                        "provider": "external",
                        "reference_images": [
                            bytes.fromhex("89504e470d0a1a0a696d616765"),
                            bytes.fromhex("ffd8ff70686f746f"),
                        ],
                        "reference_videos": [(Path(__file__).parent / "fixtures" / "iso6.mp4").read_bytes()],
...
async def test_reference_cancel_during_staging_removes_files(monkeypatch, tmp_path):
    ...
    with pytest.raises(RuntimeError, match="cancelled"):
        await provider_handler._handle_adapter_media(
            ...
            {
                "provider": "external",
                "reference_images": [bytes.fromhex("89504e470d0a1a0a696d616765")],
                "reference_videos": [],
            },
            "during-stage",
            flags,
