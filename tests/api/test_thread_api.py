...
This module tests the thread management API including:
- Thread creation
- Thread retrieval (single and list)
- Thread update
- Thread deletion with message cleanup
"""
...
from nodetool.types.thread import ThreadCreateRequest, ThreadList, ThreadUpdateRequest
...
@pytest.mark.asyncio
async def test_get_thread(client: TestClient, headers: dict[str, str], thread: Thread):
...
async def test_list_threads(client: TestClient, headers: dict[str, str], user_id: str):
    ...
    for i in range(5):
        await Thread.create(user_id=user_id, title=f"Thread {i}")
...
@pytest.mark.asyncio
async def test_update_thread(client: TestClient, headers: dict[str, str], thread: Thread):
...
@pytest.mark.asyncio
async def test_delete_thread(client: TestClient, headers: dict[str, str], thread: Thread):
...
async def test_delete_thread_with_messages(
    client: TestClient, headers: dict[str, str], thread: Thread, user_id: str
):
...
@pytest.mark.asyncio
async def test_thread_message_isolation(
    client: TestClient, headers: dict[str, str], thread: Thread, user_id: str
):
