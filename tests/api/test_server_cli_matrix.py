...
class TestWorkerCompatibilityAlias:
    def test_worker_command_routes_to_unified_server(self, monkeypatch):
        run_server_calls = []

        def mock_run_server(**kwargs):
            run_server_calls.append(
                {
                    "kwargs": kwargs,
                    "auth_provider": os.environ.get("AUTH_PROVIDER"),
                    "chat_provider": os.environ.get("CHAT_PROVIDER"),
                    "default_model": os.environ.get("DEFAULT_MODEL"),
                    "tools": os.environ.get("NODETOOL_TOOLS"),
                }
            )
        ...
        assert result.exit_code == 0
        assert len(run_server_calls) == 1
        call = run_server_calls[0]
        assert call["kwargs"]["mode"] == "private"
        assert call["kwargs"]["host"] == "0.0.0.0"
        assert call["kwargs"]["port"] == 9910
        assert call["auth_provider"] == "multi_user"
        assert call["chat_provider"] == "openai"
        assert call["default_model"] == "gpt-4o-mini"
        assert call["tools"] == "google_search,browser"
