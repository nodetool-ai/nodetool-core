...
async def test_autosave_max_versions_limit(client: TestClient, workflow: Workflow, headers: dict[str, str]):
    """Test autosave respects max versions per workflow using FIFO deletion."""
    await workflow.save()
    ...
    # Verify we can still create autosaves beyond the default max of 50
    # (FIFO deletion means we never skip, old ones are deleted)
    last_response = client.post(
        f"/api/workflows/{workflow.id}/autosave",
        json={"save_type": "autosave", "force": True},
        headers=headers,
    )
    assert last_response.json()["skipped"] is False
    assert last_response.json()["version"] is not None
