...
def test_search_assets_global_basic(user_id: str):
    """Test basic search functionality in the model layer."""
    # Create test assets
    Asset.create(user_id=user_id, name="sunset_photo", content_type="image/jpeg")
    Asset.create(user_id=user_id, name="beach_vacation", content_type="image/jpeg")
    Asset.create(user_id=user_id, name="document.txt", content_type="text/plain")
    
    # Test search
    assets, next_cursor, folder_paths = Asset.search_assets_global(
...
def test_search_assets_global_contains_search(user_id: str):
    """Test that search finds matches anywhere in filename."""
    # Create assets with query in different positions
    Asset.create(user_id=user_id, name="photo_sunset", content_type="image/jpeg")
    Asset.create(user_id=user_id, name="beautiful_photo_vacation", content_type="image/jpeg")
    Asset.create(user_id=user_id, name="vacation_photo", content_type="image/jpeg")
    
    assets, _, _ = Asset.search_assets_global(user_id=user_id, query="photo")
...
def test_search_assets_global_content_type_filter(user_id: str):
    """Test search with content type filtering."""
    Asset.create(user_id=user_id, name="test_image", content_type="image/jpeg")
    Asset.create(user_id=user_id, name="test_document", content_type="text/plain")
    
    # Search for images only
    assets, _, _ = Asset.search_assets_global(
...
def test_search_assets_global_user_isolation(user_id: str):
    """Test that search only returns current user's assets."""
    # Create asset for current user
    Asset.create(user_id=user_id, name="my_photo", content_type="image/jpeg")
...
def test_search_assets_global_sql_injection_protection(user_id: str):
    """Test that search properly sanitizes input to prevent SQL injection."""
    Asset.create(user_id=user_id, name="test_file", content_type="image/jpeg")
...
def test_search_assets_global_empty_results(user_id: str):
    """Test search with no matching results."""
    Asset.create(user_id=user_id, name="sunset", content_type="image/jpeg")
...
def test_search_assets_global_pagination(user_id: str):
    """Test search pagination."""
    # Create multiple matching assets
    for i in range(5):
        Asset.create(user_id=user_id, name=f"photo_{i}", content_type="image/jpeg")
...
def test_get_asset_path_info_batch_performance(user_id: str):
    ...
    subfolder = Asset.create(
        user_id=user_id, name="Sub Folder", content_type="folder", parent_id=folder.id
    )
    
    # Create assets in different locations
    asset1 = Asset.create(user_id=user_id, name="root_asset", content_type="image/jpeg")
    asset2 = Asset.create(
        user_id=user_id, name="folder_asset", content_type="image/jpeg", parent_id=folder.id
    )
    asset3 = Asset.create(
        user_id=user_id, name="subfolder_asset", content_type="image/jpeg", parent_id=subfolder.id
    )
    
    # Test batch path info retrieval
    asset_ids = [asset1.id, asset2.id, asset3.id]
    path_info = Asset.get_asset_path_info(user_id, asset_ids)
...
def test_get_asset_path_info_fallback_handling(user_id: str):
    """Test fallback behavior for get_asset_path_info."""
    # Test with empty list
    path_info = Asset.get_asset_path_info(user_id, [])
    assert path_info == {}
    
    # Test with non-existent asset
    path_info = Asset.get_asset_path_info(user_id, ["nonexistent_id"])
    assert path_info == {}


def test_get_asset_path_info_nested_folders(user_id: str):
    """Test path info for deeply nested folder structures."""
    # Create deep folder structure
    folder1 = Asset.create(user_id=user_id, name="Folder1", content_type="folder")
    folder2 = Asset.create(
        user_id=user_id, name="Folder2", content_type="folder", parent_id=folder1.id
    )
    folder3 = Asset.create(
        user_id=user_id, name="Folder3", content_type="folder", parent_id=folder2.id
    )
    
    # Create asset in deeply nested folder
    asset = Asset.create(
        user_id=user_id, name="deep_asset", content_type="image/jpeg", parent_id=folder3.id
    )
    
    path_info = Asset.get_asset_path_info(user_id, [asset.id])
