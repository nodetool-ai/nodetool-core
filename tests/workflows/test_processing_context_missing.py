...
from nodetool.metadata.types import (
    AssetRef, ImageRef, AudioRef, VideoRef, TextRef, ModelRef, DataframeRef
)
...
class TestPredictionAndGeneration:
    ...
    async def test_generate_message(self, context):
        ...
        with patch('nodetool.chat.providers.get_provider') as mock_get_provider:
            mock_provider = Mock()
            mock_provider.generate_message = AsyncMock(return_value=Message(role="assistant", content="Hi"))
            mock_provider.usage = {"prompt_tokens": 10, "completion_tokens": 20}
            mock_get_provider.return_value = mock_provider
            
            with patch('nodetool.chat.providers.openai_prediction.calculate_chat_cost', return_value=0.001):
                with patch('nodetool.models.prediction.Prediction.create') as mock_create:
...
class TestUtilityFunctions:
    ...
    def test_get_output_connected_nodes(self, context):
        ...
        edges = [
            Edge(source="node1", target="node2", sourceHandle="output", targetHandle="input"),
            Edge(source="node1", target="node3", sourceHandle="output", targetHandle="input")
        ]
    ...
    def test_get_output_connected_nodes_string_id(self, context):
        ...
        edges = [Edge(source="node1", target="node2", sourceHandle="output", targetHandle="input")]
    ...
    def test_get_output_connected_nodes_nonexistent(self, context):
        ...
        node1 = TestNode(id="node1")
        edges = [Edge(source="node1", target="nonexistent", sourceHandle="output", targetHandle="input")]
    ...
    async def test_is_huggingface_model_cached(self, context):
        ...
        with patch('huggingface_hub.file_download.try_to_load_from_cache') as mock_cache:
...
class TestRunWorker:
    """Test run_worker functionality."""
    
    @pytest.mark.asyncio
    async def test_run_worker_success(self, context):
        """Test successful worker execution."""
        from nodetool.workflows.run_job_request import RunJobRequest
        from nodetool.workflows.types import NodeProgress, NodeUpdate
        from nodetool.types.job import JobUpdate
        
        request = RunJobRequest(workflow_id="test_workflow")
        
        # Mock responses from worker
        mock_responses = [
            '{"type": "node_progress", "node_id": "node1", "progress": 50, "total": 100}',
            '{"type": "node_update", "node_id": "node1", "status": "running"}',
            '{"type": "job_update", "status": "completed"}'
        ]
        
        async def mock_aiter_lines():
            for line in mock_responses:
                yield line
        
        with patch.object(Environment, 'get_worker_url', return_value="http://worker:8000"):
            with patch.object(context, 'get_http_client') as mock_client:
                mock_response = Mock()
                mock_response.raise_for_status = Mock()
                ...
                mock_client_instance = Mock()
                mock_client_instance.stream = Mock(return_value=mock_stream_manager)
                mock_client.return_value = mock_client_instance
                
                result = await context.run_worker(request)
                assert result == {}
    ...
    @pytest.mark.asyncio
    async def test_run_worker_error(self, context):
        ...
        with patch.object(Environment, 'get_worker_url', return_value="http://worker:8000"):
            with patch.object(context, 'get_http_client') as mock_client:
                ...
                with pytest.raises(Exception, match="Something went wrong"):
                    await context.run_worker(request)
